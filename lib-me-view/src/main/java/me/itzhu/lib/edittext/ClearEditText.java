 ...
 * <p>
 * 清除按钮edittext
 * 如果使用setError();会出现error按钮与drawableRight重叠现象，点击会出现类似点击穿透，不过貌似没有太大影响
...
public class ClearEditText extends android.support.v7.widget.AppCompatEditText {
    ...
    public void init(AttributeSet attrs) {
        int resId = R.drawable.ic_common_edittext_clear;
        if (attrs != null) {
            for (int i = 0; i < attrs.getAttributeCount(); i++) {
                // L.e(TAG, attrs.getStyleAttribute() + "attrs get No." + i + " attr name: " + attrs.getAttributeName(i) + ", value: " + attrs.getAttributeValue(i));
                if ("drawableRight".equals(attrs.getAttributeName(i))) {
                    resId = attrs.getAttributeResourceValue(i, R.drawable.ic_common_edittext_clear);
                    break;
                }
            }
        }
